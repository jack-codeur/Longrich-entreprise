...
class Soins extends Component
{
    ...
    public function render()
    {
        ...
        $pdtSante = Produit::query();
        if ($this->search != "") {
            $pdtSante->where('nom_produit', 'LIKE', "%" .$this->search. "%")
                     ->orWhere('EstDisponible', 'LIKE', "%" .$this->search. "%");
        }

        if ($this->filterEtat != "") {
            $pdtSante->where('EstDisponible', 'LIKE', "%" .$this->filterEtat. "%");
        }

        return view('livewire.admin.soins',
        [
            '_soins' => $pdtSante->latest()->paginate(5),
    ...
    public function createProduct ()
    {
        ...
        if ($this->image_1 != "" && $this->image_2 != "" && $this->image_3 != "" && $this->image_2 != "") {
            
            $pathImage_1 = $this->image_1->store('upload', 'public');

            $FitImage_1 = Image::make(public_path('storage/'.$pathImage_1))->fit(200, 200);
            $FitImage_1->save();

            $pathImage_2 = $this->image_2->store('upload', 'public');

            $FitImage_2 = Image::make(public_path('storage/'.$pathImage_2))->fit(200, 200);
            $FitImage_2->save();

            $pathImage_3 = $this->image_3->store('upload', 'public');

            $FitImage_3 = Image::make(public_path('storage/'.$pathImage_3))->fit(200, 200);
            $FitImage_3->save();

            $pathImage_4 = $this->image_4->store('upload', 'public');
        
            $FitImage_4 = Image::make(public_path('storage/'.$pathImage_4))->fit(200, 200);
            $FitImage_4->save();
        }
    ...
    public function updateProduct(){
        ...
        $this->dispatchBrowserEvent('showSuccessMessage', ['message'=>'Produit modifié avec succè!']);
    ...
    public function confirmDelete($product_name, $id){
        $this->dispatchBrowserEvent("showConfirmMessage", [
            "message" => [
                ...
                "text" => "Vous êtes sur le point de supprimer $product_name",
                "icon" => "warning",
                "data" => [
                    "product_id" => $id
                ]
    ...
    public function deleteProduct($id){
        Produit::destroy($id);
        $this->dispatchBrowserEvent('showSuccessMessage', ['message'=>'Produit supprimé avec succè!']);
    }
    public function showAddprdtSoins(){
